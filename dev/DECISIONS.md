...
**What:** Ctrl-selecting 2-16 non-CJK tokens in page text now opens a phrase-assist tooltip instead of the single-word lookup path. The first rendering step is a compact pipe-separated ZH hint sequence that uses the best available enabled-source result for each selected token, preserving short function words such as `o`, `ko`, and `no` while hiding AB labels and source metadata. Phrase hints are phrase-mode-only compressed definitions: parentheticals are stripped, wordy definitions are split at common separators, and long chunks are ellipsized. Phrase-assist tooltips get a phrase-only width cap of 920px, while normal tooltips stay capped at 608px. Lookup runs on unique tokens with bounded concurrency and existing caches. If the popup AI tools toggle is enabled, the phrase tooltip header also shows Amis-to-ZH MT and Amis TTS buttons using the same ILRDF Gradio contracts as the saved page, with Malan defaults. MT output is appended as a separate AI line below the ZH hint sequence.
