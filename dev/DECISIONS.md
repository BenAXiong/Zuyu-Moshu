...
**What:** Ctrl-selecting 2-16 non-CJK tokens in page text now opens a phrase-assist tooltip instead of the single-word lookup path. The selected AB phrase remains in the header, where drillable words are rendered as inline header tokens. Phrase mode displays tokens of 2 characters or less as pass-through AB text in the gloss row but skips API lookup for them; longer tokens normalize curly glottals before lookup. The result row is a compact plain gloss sequence with no repeated AB labels and no source metadata; whitespace-only gaps use pipes, while punctuation found in the selected AB phrase replaces the pipe. Missing longer-token lookups render as `x`. Ambiguous tokens can show up to 2 slash-separated compact glosses. Clicking a header AB token drills into the normal tooltip lookup, and the back button can restore the original phrase-assist tooltip. Phrase hints are phrase-mode-only compressed definitions: parentheticals and MoE bracket notes are stripped, wordy definitions are split at common separators, and long chunks are ellipsized. Phrase-assist tooltips request a phrase-only 920px viewport-capped width, while normal tooltips stay capped at 608px. Lookup runs on unique tokens with bounded concurrency and existing caches. If the popup `AI工具` toggle is enabled, the phrase tooltip header also shows Amis-to-ZH MT and Amis TTS buttons using the same ILRDF Gradio contracts as the saved page, with Malan defaults. MT output is appended as a separate AI line below the ZH hint sequence and shows an in-place loading message while running.
