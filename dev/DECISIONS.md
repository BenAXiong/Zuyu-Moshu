...
**What:** The saved page `短章分析*` tab is no longer an alias of `短章分析`. It has its own input, source/language controls, bounded-concurrency lookup flow, and reader-style output. Each sentence renders as inline annotated AB text: every word remains in sentence flow, a short ZH gloss is aligned below analyzed tokens, and the top annotation slot is reserved for fallback/alt display when the looked-up form differs from the visible token. Sentence rows are visually merged into one output surface, the internal divider lines can be hidden with the `-` toggle, ZH/top annotations can be hidden independently, and the output supports full, split, and single-sentence layouts. Single mode removes the old sentence-number navigation and uses previous/next arrows around the centered sentence. Each row has right-aligned TTS, MT, and IndiHunt export buttons. Reader MT/TTS use Malan as the fallback Amis dialect. Reader IndiHunt export sends only the original AB sentence plus the sentence-level MT line when present, not the token-gloss definitions. The reader reuses the short-analysis tokenizer and lookup helpers so curly glottal normalization and Kilang best-source ranking stay consistent.
