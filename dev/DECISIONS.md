...
**What:** Ctrl-selecting 2-16 non-CJK tokens in page text now opens a phrase-assist tooltip instead of the single-word lookup path. Phrase mode displays tokens of 2 characters or less as pass-through AB text but skips API lookup for them; longer tokens normalize curly glottals before lookup. The first rendering step is a compact pipe-separated ZH hint sequence that uses the best available enabled-source result for each looked-up token while hiding AB labels and source metadata; missing longer-token lookups render as `x`. Ambiguous tokens can show up to 2 slash-separated compact glosses, and clicking a gloss drills into the normal tooltip lookup for that token. Phrase hints are phrase-mode-only compressed definitions: parentheticals are stripped, wordy definitions are split at common separators, and long chunks are ellipsized. Phrase-assist tooltips request a phrase-only 920px viewport-capped width, while normal tooltips stay capped at 608px. Lookup runs on unique tokens with bounded concurrency and existing caches. If the popup `AI工具` toggle is enabled, the phrase tooltip header also shows Amis-to-ZH MT and Amis TTS buttons using the same ILRDF Gradio contracts as the saved page, with Malan defaults. MT output is appended as a separate AI line below the ZH hint sequence and shows an in-place loading message while running.
