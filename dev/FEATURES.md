...
Global timestamp: 2026-06-06 04:50 +08:00
...
| Companion lookup | v1.6 in progress | Single-word contexts render a Side Panel lookup view. AB lookup supports Kilang and ePark/DICT paths. ZH-to-AB lookup supports enabled ePark/DICT and Kilang `moeZhLookup` sources, then renders drillable AB result rows with secondary ZH definitions. Kilang AB results show full sense definitions, examples, compact source/tier metadata, a first textual chain from root/parent/matched word, and relation labels for fallback/alt/derived matches (`~` for pure alt, branch marker plus affix pill for recovery/derived). Example AB words are drillable and example AB lines get TTS buttons when the context is Amis. Source toggles are respected; if only Kilang is enabled, Companion does not call the dictionary endpoint. | `sidepanel.js`: `buildLookupView()`, `fetchWordSections()`, `fetchZhSections()`, `fetchKilangSection()`, `fetchDictSection()`, `appendDrillableAbText()`, `createCompanionTtsButton()`, `createCompanionMtButton()`; `lookup_core.js`: Kilang, dictionary, and ZH lookup normalization helpers. |
