...
Global timestamp: 2026-06-07 17:41 +08:00
...
| Companion manual input | v1.6.2 in progress | A global input row under the Companion mode tabs lets users type a word, Chinese query, phrase, or sentence. Submit builds the same `companionContext` shape used by page selection: CJK or one AB token routes to `查詞`/word lookup, while multi-token AB text routes to `讀句`/analysis. The input uses current extension language/source settings and persists through `chrome.storage.session` like selection-triggered Companion contexts. | `sidepanel.html`: `manualSearch`; `sidepanel.js`: `handleManualSearch()`, `buildManualSearchContext()`; `sidepanel.css`: `.manual-search`. |
