...
Global timestamp: 2026-06-12 03:43 +08:00
...
| Companion AI tab (`AI`) | v1.6.3 shell | Companion now has a third persisted mode, `ai`, with its own tab and context slot. The current shell renders direction buttons, input/output textareas, and disabled Translate/Listen buttons. It intentionally does not wire MT/TTS yet; next pass will connect background ILRDF MT/TTS and decide selection/manual-input handoff behavior. | `sidepanel.html`: AI mode tab; `sidepanel.js`: `normalizeMode()`, `makeEmptyCompanionState()`, `buildAiView()`; `sidepanel.css`: `.ai-panel`, `.ai-direction`, `.ai-textarea`, `.ai-actions`. |
