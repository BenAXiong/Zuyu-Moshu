...
Global timestamp: 2026-06-06 03:56 +08:00
...
| Phrase AI tools | Amis AI, popup-gated | When the popup `AI工具` toggle is enabled, phrase-assist tooltips add header buttons for Amis-to-ZH MT and Amis TTS using Malan defaults. MT renders a separate AI line below the ZH hint sequence and shows a loading message while running. Phrase tooltip TTS now generates/caches the ILRDF audio URL in the content script, then asks the background service worker to play it through an extension offscreen document to avoid page-context playback issues. Needs a manual Chrome smoke test for actual sound. | `content.js`: `appendPhraseAiButtons()`, `translatePhrase()`, `speakPhrase()`, `gradioCall()`, `playGeneratedTtsAudio()`; `background.js`: `playOffscreenAudio()`; `offscreen.html`, `offscreen.js`; `popup.html`, `popup.js`, `shared.js`; `manifest.json`: `offscreen` permission. |
...
| ILRDF TTS playback | Amis AI | Saved-page AI TTS and `短章分析*` sentence TTS call ILRDF Gradio directly and cache returned audio URLs in memory for the current extension/page session; `短章分析*` TTS is confirmed working. Phrase-assist tooltip TTS shares the same URL generation/cache pattern, but playback is routed through an extension offscreen document because delayed `new Audio(...).play()` from a content-script tooltip was unreliable. Companion header/example TTS sends text to the background, where ILRDF audio URLs are generated/cached and then played through the same offscreen document. | `content.js`: `getTtsAudioUrl()`, `getCachedTtsAudioUrl()`, `speakPhrase()`, `playGeneratedTtsAudio()`; `background.js`: `getTtsAudioUrl()`, `playIlrdfTts()`, `ensureOffscreenAudioDocument()`, `playOffscreenAudio()`; `offscreen.html`, `offscreen.js`; `sidepanel.js`: `createCompanionTtsButton()`; `saved.js`: `getCachedTtsAudioUrl()`, `playReaderSentenceTts()`, `aiListen()`. |
...
| Feature | Scope | Current state | Implementation |
|---|---|---|---|
| Native Side Panel target | v1.6 in progress | Companion is wired as a Chrome native Side Panel. Popup can choose lookup display target `提示框 / Companion`, and eligible double-click / Ctrl-select selections route to Companion when selected. Hover remains tooltip-only/off. The content script sends raw selected context to background, background stores it in `chrome.storage.session`, opens the Side Panel, and the Side Panel renders the active lookup/analysis context. The top context block now shows only the selected title and optional TTS button by default; language/trigger/page URL/timestamp details are tucked into a collapsed disclosure. Phrase analysis places its token/lookup count in that disclosure toggle instead of a visible header pill. | `manifest.json`: `side_panel`, `sidePanel` permission; `popup.html`, `popup.js`; `content.js`: `sendCompanionContext()`; `background.js`: `openCompanion()`; `sidepanel.html`, `sidepanel.css`, `sidepanel.js`. |
| Companion lookup | v1.6 in progress | Single-word contexts render a Side Panel lookup view. Kilang results show full sense definitions, examples, compact source/tier metadata, and a first textual chain from root/parent/matched word without an extra `Kilang` source header row. Example AB words are drillable and example AB lines get TTS buttons when the context is Amis. ePark/DICT results render as full row definitions when ePark/ILRDF-like dictionary sources are enabled. Source toggles are respected; if only Kilang is enabled, Companion does not call the dictionary endpoint. | `sidepanel.js`: `buildLookupView()`, `fetchWordSections()`, `fetchKilangSection()`, `fetchDictSection()`, `appendDrillableAbText()`, `createCompanionTtsButton()`; `lookup_core.js`: Kilang and dictionary normalization helpers. |
| Companion analysis | v1.6 in progress | Phrase/sentence contexts render a basic token grid. Companion re-tokenizes the raw selection up to 80 tokens instead of inheriting the tooltip phrase cap. Tokens of 2 characters or less are pass-through and are not looked up. Longer unique tokens are looked up with bounded concurrency through enabled sources, using Kilang first by default. Each token shows a short ZH gloss and root when available. This is intentionally simpler than saved-page `短章分析*` for v1.6.0. | `sidepanel.js`: `buildAnalysisView()`, `lookupAnalysisToken()`; `lookup_core.js`: phrase tokenization and short-gloss helpers. |
| Companion drilling | v1.6 in progress | Clicking a Companion analysis token, Kilang chain node, dictionary AB line, or AB word inside a Companion example replaces the Side Panel with that word lookup. Chinese gloss/definition text is not drillable. A local `返回` button restores the previous Companion context. New page selections reset this local drill history. | `sidepanel.js`: `drillLookup()`, `goBack()`, `makeDrillButton()`, `appendDrillableAbText()`; `sidepanel.html`, `sidepanel.css`. |
| Shared lookup core | Universal helper layer | A new namespaced plain-script helper, `FDT_LOOKUP_CORE`, centralizes safe text cleaning, phrase tokenization, Kilang sense/example grouping, source metadata, short phrase gloss extraction, and dictionary row normalization for Companion. Existing tooltip code is not refactored to use it yet, to avoid changing tooltip behavior during the first Companion implementation. Future v1.6.x cleanup can migrate duplicated pure helpers into this namespace. | `lookup_core.js`; loaded by `sidepanel.html`; included in `dev/package-extension.ps1`. |
