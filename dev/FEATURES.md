...
Global timestamp: 2026-06-06 02:51 +08:00
...
| Companion drilling | v1.6 in progress | Clicking a Companion analysis token, Kilang chain node, or dictionary AB line replaces the Side Panel with that word lookup. A local `返回` button restores the previous Companion context. New page selections reset this local drill history. | `sidepanel.js`: `drillLookup()`, `goBack()`, `makeDrillButton()`; `sidepanel.html`, `sidepanel.css`. |
