...
Global timestamp: 2026-06-12 04:15 +08:00
...
| Companion AI tab (`AI`) | v1.6.3 | Companion has a third persisted mode, `ai`, with its own context slot. The tab renders direction buttons, input/output textareas, and action buttons between the boxes. Direction labels use `中文` and the selected language name, falling back to `族語` when all languages are selected. The tab supports Amis MT in both directions through background ILRDF routes: `Amis -> 中文` uses `translate`, and `中文 -> Amis` uses `translate_1`. Listen uses background `playIlrdfTts`; in `中文 -> Amis` mode it plays the generated output, while in `Amis -> 中文` mode it plays the input. All generated Companion TTS buttons use the top-bar Amis dialect selector, which is disabled unless Amis is the selected language. Non-Amis selections show explicit unsupported output for now. | `background.js`: `translateIlrdfText()`, `translateIlrdfZhToAmis()`, `playIlrdfTts()`; `sidepanel.html`: AI mode tab and `ttsDialect`; `sidepanel.js`: `normalizeMode()`, `makeEmptyCompanionState()`, `buildAiView()`, `translateAiPanel()`, `listenAiPanel()`, `getCompanionTtsSpeaker()`; `sidepanel.css`: `.ai-panel`, `.ai-direction`, `.ai-textarea`, `.ai-actions`, `.dialect-control`. |
